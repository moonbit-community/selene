...
- Fixed `selene/physics3d` rigid-body writeback robustness by skipping non-finite/invalid Rapier transforms and velocities during component sync, preventing `scene3d` dynamic spheres from flashing across the screen when physics state becomes numerically unstable.
