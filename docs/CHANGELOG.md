...
- Changed WebGPU 2D, basic 3D color, and solid shadow shader sources so MoonBit owns the WGSL text and JavaScript only creates shader modules from supplied sources.
...
- Changed WebGPU shadow view-projection handoff so JavaScript keeps MoonBit-authored shadow packets as typed arrays instead of rebuilding setup objects.
...
- Changed 2D camera pass contracts to carry explicit pass target semantics (`Screen` / `Offscreen(name)`) through `CameraPass2D -> RenderPass2D -> render_submit`, with real backend offscreen render-target execution and `offscreen://<name>` sampling support.
...
- Changed render submission flow to route every frame through `runtime_debug.decorate_frame_for_debug(...)`, enabling a global atlas overlay switch instead of game-specific debug draw systems.
...
- Changed `selene-editor/frontend/app` file organization by extracting selection/component utility functions into a dedicated helper module (`selection_component_helpers.mbt`) to reduce single-file coupling in `top.mbt`.
- Changed `selene-editor/frontend/app` reducer layout from a monolithic `top.mbt` to focused files (`top.mbt`, `messages.mbt`, `app_helpers.mbt`, `update_main.mbt`) so model/message definitions, helper utilities, and update logic are separated for ongoing maintenance.
- Changed `selene-editor/frontend/app` reducer decomposition to domain handlers (`update_project_runtime`, `update_selection_history_tools`, `update_scene_lifecycle`, `update_assets_resources`, `update_inspector_components`, `update_animation_editor`), and reduced `update_main.mbt` to transaction pre-processing plus explicit exhaustive message routing.
- Changed `selene-editor/frontend/app` reducer dispatch to a second-level ADT (`DomainMsg` + per-domain message enums + `to_domain_msg`), so handler functions now match typed domain messages directly with compile-time exhaustiveness checks and no fallback `_` routing arms.
