...
- Changed `selene-editor/frontend/app` update entrypoint to a routed reducer (`update_router.mbt`) so `update` now stays small and dispatches by message domain while legacy behavior remains in `update_main.mbt` during ongoing decomposition.
