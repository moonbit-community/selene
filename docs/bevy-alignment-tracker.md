...
| BEVY-006 | internal audit (physics3d bevy_rapier parity) | 3D physics surface parity still needs a formal gap matrix against `bevy_rapier3d` latest stable usage patterns. | IN_PROGRESS | Added baseline parity matrix at [`docs/bevy-rapier3d-parity-matrix.md`](/Users/zhengyu/Documents/projects/selene/docs/bevy-rapier3d-parity-matrix.md) and expanded wbtests for query filter (`body_filter` dynamic/fixed/kinematic, `exclude_entity`, `exclude_sensors`, `exclude_solids`, `cast_shape` filtering) plus collision/intersection event-bus mapping. Remaining work: full joint behavior matrix, controller scenarios, and cast-shape sensor/solid query edge cases. |
...
- `BEVY-006` (`IN_PROGRESS`): close remaining physics3d parity gaps from the matrix (`joint behavior`, `character controller scenarios`, `cast-shape sensor/solid edge query scenarios`).
