...
| `2D-006` | `render2d`, backend audit | 2D submission is still command-by-command with no batching/compaction, so multi-camera scenes scale poorly. | `DONE` | Added WebGPU-side 2D command compaction: adjacent compatible color/texture commands are merged before upload/draw (without crossing clip boundaries), reducing command count and buffer writes. Verified by `moon check selene-webgpu/src --target all --deny-warn`. |
