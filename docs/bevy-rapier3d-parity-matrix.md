...
| Impulse and multibody joints | fixed/revolute/prismatic/rope/spring/spherical, motor/limits/softness/contacts | `ImpulseJoint`, `MultibodyJoint`, `Joint` enum with all six types | `IN_PROGRESS` | Joint conversion paths exist in [`top.mbt`](/Users/zhengyu/Documents/projects/selene/selene-core/src/physics3d/top.mbt); wbtests now cover variant routing for all six joint kinds and `contacts_enabled` for fixed/revolute. Current implementation nuance: impulse `Rope`/`Spring` route to dedicated rope/spring solvers and do not expose `RapierImpulseJointHandle`. | Add behavior-level tests for limits/motor/softness/contacts and decide whether impulse rope/spring should expose stable handles for closer bevy_rapier parity. |
...
1. Complete joint behavior coverage (limits/motor/softness/contact toggles for all joint kinds) and resolve impulse rope/spring handle parity.
2. Complete query filter matrix (cast-shape sensor/solid exclusion scenarios).
