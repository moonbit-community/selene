...
| Impulse and multibody joints | fixed/revolute/prismatic/rope/spring/spherical, motor/limits/softness/contacts | `ImpulseJoint`, `MultibodyJoint`, `Joint` enum with all six types | `IN_PROGRESS` | Joint conversion paths exist in [`top.mbt`](/Users/zhengyu/Documents/projects/selene/selene-core/src/physics3d/top.mbt); wbtests cover variant routing for all six joint kinds on both impulse/multibody paths, `contacts_enabled` for fixed/revolute/rope/spring impulse cases, revolute/prismatic limits+motor state, and revolute `motor_model`/`softness` conversion (generic-joint path). Impulse rope/spring route through generic joints and expose stable handles. | `GenericJoint3DReal` backend still drops `motor_model`/`softness` at round-trip; keep blocked on moon_rapier backend parity and track via BEVY-008. |
...
1. Complete character controller scenario coverage (autostep/snap/slope).
2. Resolve moon_rapier 3D-real joint-state gap for `motor_model`/`softness`.
